...
Proof.
  exact (fun (H1 : A -> B -> C) (H2 : B) (H3 : A) => H1 H3 H2).
Qed.

Lemma e13_2 : (A -> B) -> (B -> C) -> A -> C.
Proof.
  exact (fun (H1 : A -> B) (H2 : B -> C) (H3 : A) => H2 (H1 H3)).
Qed.

Lemma e13_3 : (A -> B -> C) -> (B -> A) -> B -> C.
Proof.
  exact (fun (H1 : A -> B -> C) (H2 : B -> A) (H3 : B) => H1 (H2 H3) H3).
Qed.
...
Proof.
  exact (fun (H1 : A -> B -> C) (H2 : A) (H3 : B) => H1 H2 H3).
Qed.
...
Proof.
  exact (fun (H1 : forall x y: U, R x y -> R y x) (z : U) (H2 : R e z) => H1 e z H2).
Qed.
